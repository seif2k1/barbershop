...
$.each(sectionArray, function(index, value){
     ...
     $(document).scroll(function(){
         var offsetSection = $('#' + 'item-' + value).offset().top - 0;
         ...
         if ( docScroll1 >= offsetSection ){
             ...
             $('#sidebarMenu .nav-link').eq(index).addClass('active');
             $('#sidebarMenu .nav-link').eq(index).removeClass('inactive');
         }
         
     });
    
    $('.click-scroll').eq(index).click(function(e){
        var offsetClick = $('#' + 'item-' + value).offset().top - 0;
        e.preventDefault();
        $('html, body').animate({
